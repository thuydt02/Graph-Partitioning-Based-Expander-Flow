...
public class FlowNetwork {
    ...
    /*public FlowNetwork(int V, int E) {
        ...
        for (int i = 0; i < E; i++) {
            ...
            int v = rnd.nextInt(V-1);
            int w = rnd.nextInt(V-1);
            //int w = StdRandom.uniform(V);
            double capacity = rnd.nextInt(100);
            addEdge(new FlowEdge(v, w, capacity));
        }
    }*/
    
    /**
     * Initializes a random flow network with an input text file
     * file format:
     ...
     * @throws IllegalArgumentException if {@code E < 0}
     */
    public FlowNetwork(String fname) throws FileNotFoundException {        
        ...
        while (scanner.hasNextLine()){
            ...
            addEdge(new FlowEdge(Integer.parseInt(edges[0]),Integer.parseInt(edges[1]) , Integer.parseInt(edges[2])));                        
        }           
    }
    
    /**
     * Initializes a random flow network with {@code V} vertices and list of FlowEdge
     * @param V the number of vertices
     * @param feList the list of FlowEdge
     * @throws IllegalArgumentException if {@code V < 0}
     * @throws IllegalArgumentException if {@code E < 0}
     */
    public FlowNetwork(int V, ArrayList<FlowEdge> feList) {        
        if (V < 0) throw new IllegalArgumentException("Number of vertices in a Graph must be nonnegative");
        this.V = V;
        this.E = 0;
        int Es = feList.size();
        if (Es < 0) throw new IllegalArgumentException("Number of edges must be nonnegative");
        adj = (Bag<FlowEdge>[]) new Bag[V];
        for (int v = 0; v < V; v++)
            adj[v] = new Bag<FlowEdge>();                
        for (int i = 0; i < Es; i++) {            
            addEdge(feList.get(i));
        }
    }
...
*/
    ...
    public static void main(String[] args) {
        FlowEdge e = new FlowEdge(0, 1, 100, 0);
        ArrayList<FlowEdge> feList= new ArrayList(1);
        feList.add(e);        
        FlowNetwork G = new FlowNetwork(2,feList);
